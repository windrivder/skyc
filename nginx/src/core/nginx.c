...
// 每一个数组元素都是 ngx_command_t 类型，数组的结尾用 ngx_null_command 表示。
...
main(int argc, char *const *argv)
{
    ...
    // 初始化获取 error 0-NGX_SYS_NERR 对应的描述字符串
    if (ngx_strerror_init() != NGX_OK) {
        return 1;
    }

    // 解析和配置参数，保存在全局变量中
    // nginx -v => ngx_show_version = 1
    ...
    // 将命令行参数保存在 init_cycle 中方便后续使用
    if (ngx_save_argv(&init_cycle, argc, argv) != NGX_OK) {
        return 1;
    }

    // 将各种日志、配置路径初始化到 init_cycle 中
    if (ngx_process_options(&init_cycle) != NGX_OK) {
        return 1;
    }

    // 获取操作系统参数保存在全局变量中，如 ngx_pagesize，ngx_cacheline_size
    ...
    // 初始化一个做循环冗余校验的表，后续的 CRC 校验通过查表进行，提升效率
    ...
    // nginx 进行平滑升级时，它会不重启 master 进程而启动新版本的 nginx 程序；
    // master 进程会通过 execve 系统调用来启动新版本的 master 进程（先 fork 子进程再调用 exec 来运行新进程）
    // 这时旧版本的 master 进程通过环境变量来传递必要的信息
    // 新版本的 master 进程通过此函数从环境变量读取平滑升级信息，并继承旧版本的 nginx 监听的服务句柄
    ...
    // 初始化 ngx_cycle_t，解析配置文件，加载所有模块，监听端口，初始化进程间通信...
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    // 获取 NGINX 环境变量的值，如 NGINX="16000:16500:16600;"
